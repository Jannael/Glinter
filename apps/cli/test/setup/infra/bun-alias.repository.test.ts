...
describe('BunAliasRepository', () => {
	...
	it('delegates alias setup to windows adapter on windows', async () => {
		...
		vi.spyOn(repository as unknown as { getOS: () => 'windows' | 'unix' }, 'getOS').mockReturnValue('windows')

		const windowsSetAliasSpy = vi.spyOn(BunAliasWindows.prototype, 'setAlias').mockResolvedValue(undefined)
	...
	it('delegates alias setup to unix adapter on unix', async () => {
		...
		vi.spyOn(repository as unknown as { getOS: () => 'windows' | 'unix' }, 'getOS').mockReturnValue('unix')

		const windowsSetAliasSpy = vi.spyOn(BunAliasWindows.prototype, 'setAlias').mockResolvedValue(undefined)
		const unixSetAliasSpy = vi.spyOn(BunAliasUnix.prototype, 'setAlias').mockResolvedValue(undefined)
	...
	it('throws server error when windows adapter fails', async () => {
		...
		vi.spyOn(BunAliasWindows.prototype, 'setAlias').mockRejectedValue(new Error('boom'))
	...
	it('throws server error when unix adapter fails', async () => {
		...
		vi.spyOn(BunAliasUnix.prototype, 'setAlias').mockRejectedValue(new Error('boom'))
